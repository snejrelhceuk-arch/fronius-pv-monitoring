...
def api_failover_status():
    """
    Prüft, ob der Failover-Host (failover-host) erreichbar ist.
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=3) as resp:
            if resp.status == 200:
                result = {'status': 'live', 'detail': f'failover-host Web-API erreichbar'}
                _failover_cache.update(ts=now, result=result)
                return jsonify(result)
    except Exception:
        pass
    ...
    try:
        ...
        sock.close()
        result = {'status': 'reachable', 'detail': f'failover-host erreichbar (SSH), Web-API nicht'}
    except Exception:
        result = {'status': 'down', 'detail': f'failover-host ({failover_ip}) nicht erreichbar'}
