...
def menu_benachrichtigung():
    ...
    while True:
        ...
        smtp_host = getattr(config, 'NOTIFICATION_SMTP_HOST', 'smtp.strato.de')
...
def _menu_benachrichtigung_test(email: str, smtp_host: str):
    """Test-Mail senden — über Strato SMTP mit verschlüsseltem Passwort."""
    if not email:
    ...
    try:
        ...
        sender = getattr(config, 'NOTIFICATION_FROM', 'navigator@hekabe.de')
...
def _menu_benachrichtigung_password():
    ...
    smtp_user = getattr(config, 'NOTIFICATION_SMTP_USER', 'navigator@hekabe.de')
