...
- 2026-07-11: Neue Tabelle „Energiekosten nach Typ (reale Primärenergie-Kosten)" in `templates/analyse_amortisation_view.html`; Daten aus `routes/pages.py:analyse` (`energy_costs_data`). Kosten je Energieart = verbrauchte kWh × `eur_kwh_real` (PV-Gestehungskosten). Primärenergie wird aus GEMESSENEN kWh zurückgerechnet: Heizen thermisch = WP × SCOP + Heizpatrone → Holz/η; Mobilität = E-Auto-kWh → 6 l/100 km × 10 kWh/l; Haushalt = Rest (keine Hebelung). Rückwirkend WP_BASIS-Schätzung, ab 2026 gemessenes `waermepumpe_kwh`; SQL um `heizpatrone_kwh` erweitert. Modellkonstanten neu in `config.py` (`SCOP_WAERMEPUMPE`, `WIRKUNGSGRAD_HOLZKESSEL`, `BEV_VERBRAUCH_KWH_100KM`, `VERBRENNER_L_100KM`, `KRAFTSTOFF_KWH_PRO_L`). Spalte „€/kWh (real)" aus PV-Amortisationstabelle entfernt → jetzt 1. Spalte der neuen Tabelle. Tabelle wächst jährlich, tägliche Aktualisierung via `monthly_statistics`. Human-Doku: `doc/meta/PV_REFERENZSYSTEM_DOKUMENTATION.md` (Abschnitt „Energiekosten nach Typ").
