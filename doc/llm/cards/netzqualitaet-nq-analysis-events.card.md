...
changes:
	- 2026-08-08 (Spektral-API-Haertung): `/api/nq/spectral/*` begrenzt jetzt `decimate` (1..10) und `nperseg` (PSD max. 8192, STFT max. 4096), setzt eine 50-MB-Obergrenze vor Morlet-CWT und duennte Heatmaps per Bin-Mittelung statt Stride aus. `nq_spectral.decimate` kappt direkte Aufrufe ebenfalls bei Faktor 10; THD-Hinweis erklaert die erwartete Korrelation <1 wegen fehlender gerader Einzelharmonischen.
	- 2026-08-08: **Plausibilitätsprüfungen ergänzt.** Robuste Filterung extremer/ungültiger Werte in allen numerischen Berechnungen: `_series_pearson` (routes/pac4200.py), `_pearson_r` (nq/analysis/nq_hf.py), `thd_from_harmonics` + `load_harmonic_thd_series` + `load_thd_series` (nq/analysis/nq_spectral.py). NaN/Inf/Werte >1e15 (Korrelation) bzw. >1e6 (Harmonische) werden gefiltert; THD-Ergebnis validiert (<200%); Korrelationsergebnis validiert ([-1,1]).
