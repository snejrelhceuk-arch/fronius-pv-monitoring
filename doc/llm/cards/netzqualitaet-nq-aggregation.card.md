...
changes:
	- 2026-08-08 (Speicher-Reclaim eingefrorener Monate): Neues Werkzeug **`nq/aggregate/nq_prune_months.py:prune`** gibt Plattenplatz frei, den SQLite nach `DELETE` ohne `VACUUM` haelt: droppt tote `nq_agg_10s` (Legacy 10s-Arch, entfernt 2026-07-14) + loescht ephemeres `nq_raw_slow` in **eingefrorenen** Monaten (nie der laufende) und **`VACUUM`t nur bei Bedarf** (Freelist >= `--min-free-pct`, Default 20 %) → SD-schonend (~3 VACUUMs/Monat-Lebenszeit). Best-effort taeglich aus `nq/transfer/nq_primary_cap.py:run_cap` (Timer `pv-nq-primary-cap`, 00:40) — kein neuer Timer. Einmal-Reclaim 2026-08-08: `nq_2026-07.db` 236,7→055,2 MB (−181 MB); `nq/db` 503→321 MB. **Ursache** des Bloats: `nq_raw_slow`-12h-Retention greift zeilenlogisch, aber freie Seiten aus dem 1-s-Harmonik-Churn (~6,2 Mio Zeilen/12 h, ~158 MB) wurden nie zurueckgewonnen. **Projektion:** ~0,22 GB (1 a) / ~0,31 GB (5 a) / ~0,6 GB (20 a) statt ~57 GB ohne Reclaim. Doku: `doc/netzqualitaet/NQ_SPEICHER.md`.
	- 2026-08-08 (SM-Netzqualitaets-Historie permanent): Neue retention-freie Tabelle **`nq_sm_15min`** (`nq/schema/nq_primary_schema.sql`) in `nq/db/nq_YYYY-MM.db` — 15-min Vor-PAC-Netzqualitaet (L-L-Spannung + Frequenz, avg/min/max) aus dem primaeren Smart-Meter. Einmal-Werkzeug **`nq/transfer/nq_sm_backfill.py:backfill`** sammelt `data_15min` aus Backup-DBs (Voll-Archiv via `PV_NQ_SM_SOURCES`/`--source` = 07.01–06.04, GFS-Monats-Backups, Live-DB), rechnet L-N→L-L (×√3), merged je ts (Prioritaet Live→Archiv→Backups) und schreibt monatsrotiert (idempotent `INSERT OR REPLACE`, Dry-Run-Default). Ausgefuehrt 2026-08-08: 16.464 Buckets, Monate 01–06 (07.01–30.06). **Nicht** Teil der PAC-Kaskade, **nicht** von der Aggregations-Retention (`nq_5min/hourly/daily`) beruehrt. Konsumiert von `routes/pac4200.py:_nq_sm_history_rows` (Netzkriterien-Merge). Grund: `data_15min` hat nur 90-Tage-Retention → historische SM-Netzkriterien waren sonst verloren (das vermeintlich „korrupte Backup" war ein Irrtum: Daten liegen in `data_15min`, nicht `raw_data`).
