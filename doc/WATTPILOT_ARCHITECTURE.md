...
**WebSocket-Endpoint:** `ws://192.168.2.197/ws`
...
```
┌─────────────────┐
│  Wattpilot WS   │  ws://192.168.2.197/ws
└────────┬────────┘
         │ EINZIGE Verbindung
         ↓
┌─────────────────────────┐
...
async def test():
    async with websockets.connect('ws://192.168.2.197/ws', open_timeout=5) as ws:
        print('✓ Verbunden')
        msg = await ws.recv()
