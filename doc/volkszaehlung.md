# Volkszählung — PV-System Workspace

> Stand: 2026-08-04 · auto-generiert von `tools/generate_volkszaehlung.py` (pre-commit).

## Übersicht

**Gesamtgröße (Textdateien gezählt):** 1049 MB  
**Gesamtzeilen (Code/Doku/Daten):** 276.037 Zeilen  
**Gezählte Dateien:** 525  
**Ausgeschlossene Verzeichnisse:** 22 (.venv, __pycache__, node_modules, .git …)

## Nach Sprache/Typ

| Sprache/Typ | Dateien | Zeilen | Anteil |
|---|---|---|---|
| **CSV** | 29 | 165.665 | 60.0% |
| **Python** | 177 | 54.946 | 19.9% |
| **Markdown** | 138 | 24.137 | 8.7% |
| **HTML** | 20 | 15.013 | 5.4% |
| **Shell** | 59 | 4.662 | 1.7% |
| **JSON** | 19 | 4.070 | 1.5% |
| **TXT** | 28 | 2.969 | 1.1% |
| **SQL** | 10 | 1.387 | 0.5% |
| **CSS** | 4 | 1.108 | 0.4% |
| **JavaScript** | 4 | 905 | 0.3% |
| **YAML** | 3 | 562 | 0.2% |
| **systemd** | 31 | 538 | 0.2% |
| **CONF** | 2 | 51 | 0.0% |
| **TOML** | 1 | 24 | 0.0% |
| **Total** | 525 | 276.037 | 100% |

## Python-Code nach ABCDEN-Rollen

| Rolle | Verzeichnis | .py-Dateien | Zeilen |
|---|---|---|---|
| **C** Automation | `automation/` | 41 | 14.363 |
| **A** Collector | `collector/` | 18 | 4.689 |
| **B** Web-API | `routes/` | 19 | 9.875 |
| **D** Diagnos | `diagnos/` | 5 | 1.619 |
| **E** Steuerbox | `steuerbox/` | 5 | 1.235 |
| **N** Netzqualität | `nq/` | 25 | 4.800 |
| **N** Netzqualität (Legacy) | `netzqualitaet/` | 4 | 1.462 |

## Größte Python-Dateien (Top 10)

1. `automation/engine/regeln/geraete.py` — 2.300 Zeilen
1. `solar_geometry.py` — 1.980 Zeilen
1. `routes/verbraucher.py` — 1.352 Zeilen
1. `pv-config.py` — 1.324 Zeilen
1. `solar_forecast.py` — 1.143 Zeilen
1. `automation/engine/regeln/waermepumpe.py` — 1.132 Zeilen
1. `automation/engine/event_notifier.py` — 1.046 Zeilen
1. `automation/engine/regeln/soc_steuerung.py` — 1.037 Zeilen
1. `routes/pac4200.py` — 956 Zeilen
1. `routes/visualization.py` — 951 Zeilen

## Ausgeschlossene Bereiche

`.git/`, `.venv/`, `__pycache__/`, `node_modules/`, Cache-Verzeichnisse sowie Binärdateien (nur Text-/Code-Endungen werden gezählt).

